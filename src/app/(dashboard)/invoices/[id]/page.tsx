  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-6 text-slate-900 sm:px-6 lg:px-8">
      ...
      <div className="no-print mx-auto mb-5 flex max-w-4xl flex-wrap items-center justify-between gap-3">
        <Link href="/invoices" className="text-sm font-bold text-slate-600 hover:text-blue-700">← {t.invoice.backToInvoices}</Link>
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={shareViaWhatsApp} className="min-h-11 rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-emerald-700">{t.actions.shareWhatsApp}</button>
          <button type="button" onClick={duplicateInvoice} className="min-h-11 rounded-xl bg-white px-4 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50">{t.actions.duplicate}</button>
          <button type="button" onClick={() => void changeStatus(invoiceStatus === 'paid' ? 'unpaid' : 'paid')} disabled={savingStatus} className="min-h-11 rounded-xl bg-white px-4 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50 disabled:opacity-60">{savingStatus ? t.actions.saving : invoiceStatus === 'paid' ? t.actions.markUnpaid : t.actions.markPaid}</button>
          <Link href={`/invoices/new?id=${invoice.id}`} className="inline-flex min-h-11 items-center rounded-xl bg-blue-700 px-4 text-sm font-bold text-white shadow-sm hover:bg-blue-800">Edit</Link>
          <button type="button" onClick={printInvoice} className="min-h-11 rounded-xl bg-slate-900 px-4 text-sm font-bold text-white shadow-sm hover:bg-slate-800">Print / PDF</button>
        </div>
      </div>
      ...
      <article className="invoice-paper mx-auto max-w-4xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-10">
        <header className="flex flex-col justify-between gap-6 border-b border-slate-200 pb-6 sm:flex-row">
          <div className="flex items-start gap-4">
            {company?.logo_url && <img src={company.logo_url} alt="Company logo" crossOrigin="anonymous" className="h-16 w-16 object-contain" />}
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">{companyRegNo}{company?.sst_no ? ` | SST: ${company.sst_no}` : ''}</p>
            </div>
          </div>
        ...
        <section className="grid gap-6 border-b border-slate-200 py-6 sm:grid-cols-2">
          <div>
            ...
            <p className="whitespace-pre-line text-sm text-slate-500">{company?.address || 'Billing Department'}</p>
          ...
          <div className="sm:text-right">
            ...
            <p className="mt-2 font-bold">{invoice.customer?.company_name || invoice.customer?.name || 'Customer'}</p>
            {invoice.customer?.name &&
              invoice.customer?.company_name &&
              invoice.customer.name.trim().toLowerCase() !== invoice.customer.company_name.trim().toLowerCase() && (
        ...
        <section className="ml-auto max-w-sm border-t border-slate-200 pt-4">
          <div className="flex justify-between py-1.5 text-sm"><span className="text-slate-500">Subtotal</span><span className="font-semibold">{formatMoney(invoice.subtotal_amount)}</span></div>
          <div className="flex justify-between py-1.5 text-sm"><span className="text-slate-500">Discount</span><span className="font-semibold">-{formatMoney(invoice.less_amount)}</span></div>
          <div className="mt-2 flex justify-between border-t-2 border-slate-900 pt-3 text-lg"><span className="font-bold">Total Payable</span><span className="font-bold">{formatMoney(invoice.total_amount)}</span></div>
        </section>

        <footer className="mt-8 border-t border-slate-200 pt-4 text-sm text-slate-500">
          <p>Thank you for your business.</p>
          <p className="mt-1 font-medium text-slate-700">Payment Terms: {invoice.payment_terms || company?.payment_terms || '30 Days'}</p>
          ...
          <div className="mt-10 flex items-end justify-between gap-8">
            {invoice.requires_customer_signature ? (
              <div className="flex flex-col items-start">
                ...
                <div className="mt-2 h-16 w-48"></div>
                <div className="w-48 border-b border-slate-300"></div>
                <p className="mt-1 text-xs font-semibold text-slate-500">Customer Acceptance / Stamp</p>
              </div>
            ) : (
              <div></div>
            )}
            ...
            <div className="flex flex-col items-end text-right">
              ...
              <div className="ml-auto w-48 border-b border-slate-300"></div>
