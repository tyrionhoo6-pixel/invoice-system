...
export default function InvoicesPage() {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return invoices.filter((invoice) => {
      ...
      const matchesMonth = !month || monthKey(invoice.created_at) === month;

      return matchesQuery && matchesStatus && matchesCustomer && matchesMonth;
    });
  ...
  const exportInvoices = () => {
    ...
    if (exportMode === 'outstanding_only') {
      ...
      exportRows(outstandingRows, `outstanding_invoices_${selectedCustomerName}_${today}.csv`);
    } else if (exportMode === 'monthly') {
      const selectedMonth = month || today.slice(0, 7);
      exportRows(
        invoices.filter((invoice) => monthKey(invoice.created_at) === selectedMonth),
        `invoices_monthly_${selectedMonth}.csv`
      );
      ...
      exportRows(filteredInvoices, `statement_${selectedCustomerName}_${today}.csv`);
    }
  };
  ...
  const confirmVoid = async () => {
    ...
    try {
      ...
      setInvoices((prev) =>
        prev.map((inv) => (inv.id === updated.id ? { ...inv, status: 'VOID' } : inv))
      );
                      ...
                      <tr key={invoice.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-3 py-4">
                          <p className="font-semibold">
                            {invoice.customer?.name ?? t.invoice.customer}
                          </p>
                          <p className="text-xs text-slate-500">
                            {invoice.customer?.company_name ?? t.customer.noCompany}
                          </p>
                        </td>
