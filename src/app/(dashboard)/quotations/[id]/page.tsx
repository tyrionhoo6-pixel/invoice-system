...
export default function QuotationDetailPage() {
  ...
  useEffect(() => {
    const loadQuotation = async () => {
      try {
        const result = await QuotationService.getQuotationById(params.id);
        const companySettings = await QuotationService.getCompanySettings();
        if (!result) setErrorMessage('Quotation not found.');
        setQuotation(result);
    ...
    if (params.id) void loadQuotation();
  }, [params.id]);
  ...
  const changeStatus = async (status: 'accepted' | 'rejected' | 'pending') => {
    ...
    setErrorMessage('');
    try {
      setQuotation({ ...quotation, ...(await QuotationService.updateQuotationStatus(quotation.id, status)) });
    } catch (error) {
  ...
  if (loading) return <main className="min-h-screen bg-slate-50 px-4 py-10 text-center text-slate-500">Loading...</main>;
  if (!quotation) return <main className="min-h-screen bg-slate-50 px-4 py-10 text-center"><p className="font-semibold text-red-700">{errorMessage || 'Quotation not found.'}</p><Link href="/quotations" className="mt-4 inline-flex font-bold text-blue-700">Back to Quotations</Link></main>;
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-6 text-slate-900 sm:px-6 lg:px-8">
      ...
      <div className="no-print mx-auto mb-5 flex max-w-4xl flex-wrap items-center justify-between gap-3">
        <Link href="/quotations" className="text-sm font-bold text-slate-600 hover:text-blue-700">← Back to Quotations</Link>
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" onClick={shareViaWhatsApp} className="min-h-11 rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-emerald-700">Share WhatsApp</button>
          <button type="button" onClick={duplicateQuotation} className="min-h-11 rounded-xl bg-white px-4 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50">Duplicate</button>
          <Link href={`/quotations/new?id=${quotation.id}`} className="inline-flex min-h-11 items-center rounded-xl bg-slate-800 px-4 text-sm font-bold text-white shadow-sm hover:bg-slate-900">Edit</Link>
          <button type="button" onClick={printQuotation} className="min-h-11 rounded-xl bg-slate-900 px-4 text-sm font-bold text-white shadow-sm hover:bg-slate-800">Print / PDF</button>
        </div>
      </div>
      ...
      <article className="invoice-paper mx-auto max-w-4xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-10">
        ...
        <header className="flex flex-col justify-between gap-6 border-b border-slate-200 pb-6 sm:flex-row">
          <div className="flex items-start gap-4">
            {company?.logo_url && (
              <img src={company.logo_url} alt="Company logo" crossOrigin="anonymous" className="h-16 w-16 object-contain" />
            )}
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.2em] text-blue-700">{company?.company_name || 'InvoiceSys'}</p>
        ...
        <section className="grid gap-6 border-b border-slate-200 py-6 sm:grid-cols-2">
          ...
          <div className="sm:text-right">
            ...
            <p className="mt-2 font-bold">{quotation.customer?.company_name || quotation.customer?.name || 'Customer'}</p>
            {quotation.customer?.name &&
              quotation.customer?.company_name &&
              quotation.customer.name.trim().toLowerCase() !== quotation.customer.company_name.trim().toLowerCase() && (
                <p className="text-sm text-slate-500">Attn: {quotation.customer.name}</p>
              )}
            {quotation.customer?.address && <p className="whitespace-pre-line text-sm text-slate-500">{quotation.customer.address}</p>}
            {quotation.customer?.phone && <p className="text-sm text-slate-500">{quotation.customer.phone}</p>}
          </div>
        </section>
        ...
        <section className="ml-auto max-w-sm border-t border-slate-200 pt-4">
          <div className="flex justify-between py-1.5 text-sm"><span className="text-slate-500">Subtotal</span><span className="font-semibold">{formatMoney(quotation.subtotal_amount)}</span></div>
          <div className="flex justify-between py-1.5 text-sm"><span className="text-slate-500">Discount</span><span className="font-semibold">-{formatMoney(quotation.less_amount)}</span></div>
          <div className="mt-2 flex justify-between border-t-2 border-slate-900 pt-3 text-lg"><span className="font-bold">Total Amount</span><span className="font-bold">{formatMoney(quotation.total_amount)}</span></div>
        </section>

        {/* Footer 页脚区域 */}
        <footer className="mt-8 border-t border-slate-200 pt-4 text-sm text-slate-500">
          <p>Thank you for your business.</p>
          <p className="mt-1 font-medium text-slate-700">Payment Terms: {quotation.payment_terms || company?.payment_terms || '30 Days'}</p>
          ...
          {(company?.bank_name || company?.bank_account_no || company?.bank_account_holder) && (
            <div className="mt-3 text-sm text-slate-700 space-y-0.5">
              {company.bank_name && <p><span className="font-semibold">Bank Name:</span> {company.bank_name}</p>}
              {company.bank_account_no && <p><span className="font-semibold">Bank Account:</span> {company.bank_account_no}</p>}
              {company.bank_account_holder && <p><span className="font-semibold">Bank Holder:</span> {company.bank_account_holder}</p>}
            </div>
          )}

          {/* 签名区域：左边 Customer Sign，右边 Authorized Signature */}
          <div className="mt-10 grid grid-cols-2 gap-8 pt-4">
            ...
            <div className="flex flex-col items-start justify-end">
              ...
              <p className="mt-1 text-xs font-semibold text-slate-500">Customer Acceptance / Signature</p>
            </div>

            {/* 公司授权签字区 */}
            <div className="flex flex-col items-end justify-end text-right">
              <p className="text-xs font-bold uppercase text-slate-600">{company?.company_name || 'InvoiceSys'}</p>
              ...
              <div className="w-48 border-b border-slate-300 ml-auto"></div>
        ...
        {/* 状态更新与交互（网页操作，不打印） */}
        <div className="no-print mt-8 border-t border-slate-200 pt-6 flex items-center justify-between">
          <div>
            <span className="text-sm font-semibold text-slate-500">Status: </span>
            <span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${
              quotationStatus === 'accepted' ? 'bg-emerald-50 text-emerald-700' : quotationStatus === 'rejected' ? 'bg-red-50 text-red-700' : quotationStatus === 'converted' ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-700'
            }`}>
      ...
      <div className="no-print mx-auto mt-5 max-w-4xl text-center text-xs text-slate-500">Use your browser&apos;s print dialog to save this quotation as a PDF.</div>
    </main>
  );
}